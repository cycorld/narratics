  ...
  <style>
    ...
    * {
    ...
    textarea.manuscript-editor.typewriter-mode {
      /* Instant deterministic alignment for typing; smooth scroll handled conditionally in JS */
    }
    ...
    /* ========================================================
       Mobile Responsive & Ergonomic Layout (<= 768px)
       ======================================================== */
    @media (max-width: 768px) {
      ...
      .brand-logo span,
      #btn-open-library span,
      #btn-open-new-book span,
      #active-book-genre,
      #sync-status-text,
      #total-progress-pill,
      #btn-open-cmd-palette span,
      #btn-open-cmd-palette kbd,
      #btn-zen-mode span,
      #btn-zen-mode kbd,
      #btn-typewriter-mode span,
      #btn-typewriter-mode kbd,
      #btn-open-typo-menu span,
      .top-actions kbd {
      ...
      aside.binder-panel.mobile-open {
      ...
      aside.inspector-panel.mobile-open {
        transform: translateX(0);
      }
      ...
      #btn-toggle-split, #btn-toggle-scrivenings {
        display: none !important;
      }
    ...
    @media (min-width: 769px) {
      .mobile-quick-bar, .mobile-backdrop {
        display: none !important;
      }
    }
  ...
  <div class="app-workspace">
    ...
    <main class="studio-panel">
      ...
      <div class="mobile-quick-bar" id="mobile-quick-bar">
        <button class="quick-btn" id="m-btn-binder" title="목차 열기">
        ...
        <button class="quick-btn" id="m-btn-mention" title="@ 로어 멘션">
          <span style="font-weight:700; font-size:15px; line-height:1;">@</span>
          <span>멘션</span>
        </button>
        <button class="quick-btn" id="m-btn-typewriter" title="타자기 45%">
          <svg class="icon" viewBox="0 0 24 24"><polyline points="4 7 4 4 20 4 20 7"></polyline><line x1="9" y1="20" x2="15" y2="20"></line><line x1="12" y1="4" x2="12" y2="20"></line></svg>
          <span>타자기</span>
        </button>
        <button class="quick-btn" id="m-btn-corkboard" title="코르크보드">
        ...
        <button class="quick-btn" id="m-btn-inspector" title="세계관/스냅샷">
          <svg class="icon" viewBox="0 0 24 24"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"></path><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"></path></svg>
          <span>세계관</span>
        </button>
      </div>
  ...
  <script>
    ...
    function renderBinder(searchQuery = "") {
      ...
      rootFolders.forEach(folder => {
        ...
        folderHeader.setAttribute("data-id", folder.id);
        ...
        childScenes.forEach((scene, sIdx) => {
          ...
          sceneDiv.setAttribute("data-id", scene.id);
    ...
    function updateTypewriterLayout() {
      ...
      if (isTypewriterMode) {
        ...
        const padBottom = Math.floor(clientH * 0.85);
    ...
    function scrollTypewriter(smooth = false) {
      ...
      if (!clientH) return;
      const padTop = Math.floor(clientH * 0.45);
      if (textarea.style.paddingTop !== padTop + "px") {
        updateTypewriterLayout();
      }
    ...
    // Sidebars & Mobile Drawer Controls
    const mobileBackdrop = document.getElementById("mobile-backdrop");

    function closeMobileDrawers() {
      binderPanel.classList.remove("mobile-open");
      inspectorPanel.classList.remove("mobile-open");
      if (mobileBackdrop) mobileBackdrop.classList.remove("visible");
      const mb = document.getElementById("m-btn-binder");
      const mi = document.getElementById("m-btn-inspector");
      if (mb) mb.classList.remove("active");
      if (mi) mi.classList.remove("active");
    }

    function toggleMobileBinder() {
      if (window.innerWidth <= 768) {
        const isOpen = binderPanel.classList.toggle("mobile-open");
        inspectorPanel.classList.remove("mobile-open");
        const mb = document.getElementById("m-btn-binder");
        const mi = document.getElementById("m-btn-inspector");
        if (mi) mi.classList.remove("active");
        if (mb) mb.classList.toggle("active", isOpen);
        if (mobileBackdrop) mobileBackdrop.classList.toggle("visible", isOpen);
      } else {
    ...
    function toggleMobileInspector() {
      if (window.innerWidth <= 768) {
        ...
        const mi = document.getElementById("m-btn-inspector");
        if (mb) mb.classList.remove("active");
        if (mi) mi.classList.toggle("active", isOpen);
        if (mobileBackdrop) mobileBackdrop.classList.toggle("visible", isOpen);
      } else {
    ...
    document.getElementById("btn-toggle-binder").addEventListener("click", toggleMobileBinder);
    document.getElementById("btn-toggle-inspector").addEventListener("click", toggleMobileInspector);
    const mBtnBinder = document.getElementById("m-btn-binder");
    if (mBtnBinder) mBtnBinder.addEventListener("click", toggleMobileBinder);
    const mBtnInspector = document.getElementById("m-btn-inspector");
    if (mBtnInspector) mBtnInspector.addEventListener("click", toggleMobileInspector);
    if (mobileBackdrop) mobileBackdrop.addEventListener("click", closeMobileDrawers);

    // Mobile Quick Bar: Mention trigger
    const mBtnMention = document.getElementById("m-btn-mention");
    if (mBtnMention) {
      mBtnMention.addEventListener("click", () => {
        closeMobileDrawers();
        editorTextarea.focus();
        const pos = editorTextarea.selectionStart;
        const text = editorTextarea.value;
        editorTextarea.value = text.slice(0, pos) + "@" + text.slice(pos);
        editorTextarea.setSelectionRange(pos + 1, pos + 1);
        editorTextarea.dispatchEvent(new Event("input", { bubbles: true }));
      });
    }

    // Mobile Quick Bar: Typewriter toggle
    const mBtnTypewriter = document.getElementById("m-btn-typewriter");
    if (mBtnTypewriter) {
      mBtnTypewriter.addEventListener("click", () => {
        document.getElementById("btn-typewriter-mode").click();
        mBtnTypewriter.classList.toggle("active", isTypewriterMode);
      });
    }

    // Mobile Quick Bar: Corkboard view toggle
    const mBtnCorkboard = document.getElementById("m-btn-corkboard");
    if (mBtnCorkboard) {
      mBtnCorkboard.addEventListener("click", () => {
        closeMobileDrawers();
        if (viewMode === "editor") {
          document.getElementById("btn-view-corkboard").click();
          mBtnCorkboard.classList.add("active");
        } else {
          document.getElementById("btn-view-editor").click();
          mBtnCorkboard.classList.remove("active");
        }
      });
    }
